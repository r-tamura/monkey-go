...
func TestDefine(t *testing.T) {
	...
	if d != expected["d"] {
		t.Errorf("expected b=%+v, got=%+v", expected["d"], d)
	}
	...
	if f != expected["f"] {
		t.Errorf("expected b=%+v, got=%+v", expected["f"], f)
	}
}
...
func TestResolveBuitin(t *testing.T) {
	global := NewSymbolTable()
	firstLocal := NewEnclosedSymbolTable(global)
	secondLobal := NewEnclosedSymbolTable(global)
	...
	for _, table := range []*SymbolTable{global, firstLocal, secondLobal} {
		for _, sym := range expected {
